...
class ProyekController extends Controller
{
	...
	public function save(Request $request) {
		...
		$Proyek->simpan($data['kategori_id'], $getfilename, $data['nama_project'], $data['deskripsi'], $slug_Proyek);

		if ($request->hasFile('sub_img')) {
			foreach ($request->file('sub_img') as $img) {
				$name = $img->getClientOriginalName();
				$getfilename =  str_replace(' ', '_', $name);
	...
	public function update(Request $request, $id) {
		...
		if($request->hasFile('featured_image')) {
            ...
            $getfilename = Proyek::find($id)->featured_image;
        }

        $slug_Proyek = str_slug($data['nama_project'], '-');
		$Proyek->ubah_data($data['kategori_id'], $getfilename, $data['nama_project'], $data['deskripsi'], $slug_Proyek, $id);
		...
		if (Proyek::find($id)->gambar->count() > 0) {
			...
			if ($data['sub_img'] != null) {
				if (Proyek::find($id)->gambar->count() > count($data['sub_img'])) {
					for ($i=0; $i < Proyek::find($id)->gambar->count(); $i++) {
