...
<table class="table table-bordered datatable" id="table-1">
	...
	<tbody>
		...
		<tr class="odd gradeX">
			...
			<td><a href="{{route('event.single', $event->slug)}}" target="__blank">{{strtoupper($event->title)}}</a></td>
			<td>{{ date('M j, Y', strtotime($event->created_at)) }} | <b>Telah Terbit</b></td>
			<td>
				<div align="center">
					<form action="{{route('delete_event', ['id' => $event->id_event])}}" method="post">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<a href="{{route('edit_event', $event->id_event)}}" class="btn btn-sm btn-green btn-icon icon-left">
						<i class="entypo-pencil"></i>
							Ubah
						</a>
						<button type="submit" onclick="return confirm('Anda Yakin Akan Menghapus event Ini ?')"class="btn btn-sm btn-danger btn-icon icon-left">
