...
@section('title','Semua Kategori Project')
@section('main')
...
<table class="table table-bordered datatable" id="table-1">
	...
	<tbody>
		<?php $no = 0; ?>
		@foreach($kprojects as $row)
		<tr class="odd gradeX">
			<td><center>{{++$no}}</center></td>
			<td>{{$row->nama_kategori}}</td>
			<td>
				<div align="center">
					<form action="{{route('delete_proyek', $row->id_kategori)}}" method="post">
						<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<a href="{{route('edit_kproyek', $row->id_kategori)}}" class="btn btn-sm btn-green btn-icon icon-left">
