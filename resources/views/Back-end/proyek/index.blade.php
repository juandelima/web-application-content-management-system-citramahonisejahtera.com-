...
<table class="table table-bordered datatable" id="table-1">
	...
	<tbody>
		...
		<tr class="odd gradeX">
			...
			<td><a href="{{route('projects.single', $row->slug)}}" target="__blank">{{strtoupper($row->nama_project)}}</a></td>
			<td>{{$row->kategori->nama_kategori}}</td>
			<td>
				<div align="center">
					<form action="{{route('delete_a_proyek', ['id' => $row->id_project])}}" method="post">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<a href="{{route('edit_proyek', $row->id_project)}}" class="btn btn-sm btn-green btn-icon icon-left">
						<i class="entypo-pencil"></i>
							Ubah
						</a>
						<button type="submit" onclick="return confirm('Anda Yakin Akan Menghapus Proyek Ini !')"class="btn btn-sm btn-danger btn-icon icon-left">
